...
// (3) More distinct colors - replaced duplicate blue with green, more vivid borders
const BG_COLORS = [
  { id: 'emerald', bg: '#064e3b', border: '#059669' },
  { id: 'slate', bg: '#1e293b', border: '#475569' },
  { id: 'stone', bg: '#44403c', border: '#78716c' },
  { id: 'wine', bg: '#4a1d2e', border: '#9f1239' },
  { id: 'navy', bg: '#172554', border: '#1d4ed8' },
];
...
// Fun Stats - (5) removed wall, added uniquePairs for connections
const calcFunStats = (matches, players, ratings, leaderboard, sortedMatches) => {
  ...
  return {
    hotStreak, biggestUpset, nemesis, shutoutKing, clutchPlayer, uniquePairs,
...
// (11) Bigger avatars, more vivid borders
const PlayerAvatar = ({player,size=36}) => {
  if(!player?.avatar){const ini=(player?.name||'?').split(' ').map(w=>w[0]).join('').slice(0,2).toUpperCase();return <div style={{width:size,height:size,borderRadius:'50%',background:'#1a1a1a',border:'2px solid #333',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,fontSize:size*.35,fontWeight:600,color:'#666'}}>{ini}</div>;}
  const fn=ANIMALS[player.avatar]||ANIMALS.fox;
  const bg=BG_COLORS.find(b=>b.id===player.avatar_bg)||BG_COLORS[0];
  return <div style={{width:size,height:size,borderRadius:'50%',background:bg.bg,border:`2.5px solid ${bg.border}`,display:'flex',alignItems:'center',justifyContent:'center',color:'#e5e5e5',flexShrink:0}}>{fn(size*.55)}</div>;
};

const ScoreStepper = ({label,value,onChange}) => (
  <div style={{textAlign:'center'}}>
    <div style={{color:'#999',fontSize:'13px',marginBottom:'10px',letterSpacing:'0.05em',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',fontWeight:500}}>{label}</div>
...
// (9) Higher contrast - darker base
const Wrapper = ({children}) => (
  <div style={{minHeight:'100vh',background:'#050505',position:'relative'}}>
    <div style={{position:'fixed',inset:0,background:'radial-gradient(ellipse at 50% 0%, #0c1629 0%, #050505 55%)',opacity:0.7,pointerEvents:'none',zIndex:0}}/>
...
// (8) Match result toast with PPR changes + upset celebration
const MatchToast = ({data,onDone}) => {
  useEffect(()=>{const t=setTimeout(onDone,4000);return()=>clearTimeout(t);},[onDone]);
  if(!data) return null;
  const isUpset = data.ratingDiff > 50 && data.winnerWasUnderdog;
  return <div style={{position:'fixed',top:'50%',left:'50%',transform:'translate(-50%,-50%)',zIndex:1100,textAlign:'center',animation:'toastIn .4s ease-out'}} onClick={onDone}>
    <div style={{background:'#151515',border:'1px solid #2a2a2a',borderRadius:'20px',padding:'32px 40px',minWidth:'260px',boxShadow:'0 20px 60px rgba(0,0,0,0.7)'}}>
      {isUpset&&<div style={{fontSize:'32px',marginBottom:'8px'}}>🚨 UPSET!</div>}
      <div style={{fontSize:'15px',color:'#999',marginBottom:'8px',fontWeight:500}}>{isUpset?'':'Match Recorded'}</div>
      <div style={{fontSize:'20px',fontWeight:600,color:'#fff',marginBottom:'4px'}}>{data.winnerName} wins!</div>
      <div style={{fontSize:'28px',fontWeight:300,color:'#888',margin:'8px 0',fontVariantNumeric:'tabular-nums'}}>{data.score}</div>
      <div style={{display:'flex',justifyContent:'center',gap:'24px',marginTop:'12px'}}>
        <div><span style={{color:'#34d399',fontWeight:600,fontSize:'16px'}}>+{data.winnerGain}</span><div style={{fontSize:'11px',color:'#666',marginTop:'2px'}}>{data.winnerName.split(' ')[0]}</div></div>
        <div><span style={{color:'#f87171',fontWeight:600,fontSize:'16px'}}>{data.loserGain}</span><div style={{fontSize:'11px',color:'#666',marginTop:'2px'}}>{data.loserName.split(' ')[0]}</div></div>
      </div>
    </div>
  </div>;
};
...
export default function App() {
  ...
  const [editingMatch,setEditingMatch]=useState(null);const [emW1,setEmW1]=useState(0);const [emW2,setEmW2]=useState(0);
  ...
  const playersWithMatches = useMemo(()=>leaderboard.filter(p=>p.totalMatches>0),[leaderboard]);
  const chartData = useMemo(()=>{const all=genCalendar(history,players,playersWithMatches);return chartRange===0?all:all.slice(-chartRange);},[chartRange,history,players,playersWithMatches]);
  ...
  // (4+8) Add match with immediate refresh + toast showing PPR change
  const handleAddMatch = async () => {
    if(!mp1||!mp2||mp1===mp2||(!mw1&&!mw2)) return;
    const oldR1 = ratings[mp1]||1500, oldR2 = ratings[mp2]||1500;
    let r1=oldR1, r2=oldR2;
    for(let i=0;i<mw1;i++){const e=calcExp(r1,r2);const d=Math.round(K_FACTOR*(1-e));r1+=d;r2-=d;}
    for(let i=0;i<mw2;i++){const e=calcExp(r2,r1);const d=Math.round(K_FACTOR*(1-e));r2+=d;r1-=d;}
    const p1=players.find(p=>p.id===mp1),p2=players.find(p=>p.id===mp2);
    const winnerIs1=mw1>mw2;
    setSaving(true);
    const {error} = await supabase.from('matches').insert({player1_id:mp1,player2_id:mp2,player1_wins:mw1,player2_wins:mw2});
    setSaving(false);
    if(error){alert('Error: '+error.message);return;}
    notifySlack(`🏓 ${p1?.name} ${mw1}–${mw2} ${p2?.name} — ${winnerIs1?p1?.name:p2?.name} wins!`);
    setMatchToast({
      winnerName:winnerIs1?p1?.name:p2?.name, loserName:winnerIs1?p2?.name:p1?.name,
      score:`${mw1}–${mw2}`, winnerGain:winnerIs1?r1-oldR1:r2-oldR2, loserGain:winnerIs1?r2-oldR2:r1-oldR1,
      winnerWasUnderdog:winnerIs1?oldR1<oldR2:oldR2<oldR1, ratingDiff:Math.abs(oldR1-oldR2)
    });
  ...
  // (4) Immediate refresh after avatar save
  const handleSaveAvatar = async () => {
    ...
    setSaving(false);setAvatarPickerFor(null);
    await loadData();
  };
  ...
  const handleDeletePlayer = async (playerId) => {
    ...
    setSelectedPlayer(null);await loadData();
  };

  const handleSaveCities = async (nc) => { setCities(nc); await supabase.from('app_settings').upsert({id:'cities',value:nc}); };
  const handleSaveTeams = async (nt) => { setTeams(nt); await supabase.from('app_settings').upsert({id:'teams',value:nt}); };

  const openEditPlayer = (p) => { setEditingPlayer(p);setEpName(p.name);setEpAlias(p.alias||'');setEpCity(p.city||'Seattle');setEpTeam(p.team||'GRAISE');setEpError(''); };
  const handleEditPlayer = async () => {
    if(!epName.trim()){setEpError('Name required');return;}
    setSaving(true);
    const alias = epAlias.includes('@')?epAlias.split('@')[0]:epAlias;
    await supabase.from('players').update({name:epName.trim(),alias:alias.trim(),city:epCity,team:epTeam}).eq('id',editingPlayer.id);
    setSaving(false);setEditingPlayer(null);await loadData();
  };

  // (10) Edit match scores
  const openEditMatch = (m) => { setEditingMatch(m);setEmW1(m.player1_wins);setEmW2(m.player2_wins); };
  const handleEditMatch = async () => {
    if(!editingMatch) return;
    setSaving(true);
    await supabase.from('matches').update({player1_wins:emW1,player2_wins:emW2}).eq('id',editingMatch.id);
    setSaving(false);setEditingMatch(null);await loadData();
  };
  ...
  const lbl={display:'block',fontSize:'11px',color:'#666',letterSpacing:'0.05em',marginBottom:'8px',fontWeight:500};
  ...
  if(selectedPlayer){
    ...
    return <Wrapper>
      <div style={{padding:'20px 0 16px',position:'sticky',top:0,zIndex:50}}>
        <div style={{fontSize:'22px',fontWeight:700,letterSpacing:'-0.03em',color:'#fff'}}>GRAISE <span style={{fontWeight:300,color:'#666'}}>PPR</span></div>
      </div>
      <div style={{paddingTop:'8px',paddingBottom:'40px'}}>
        <button onClick={()=>setSelectedPlayer(null)} style={{background:'none',border:'none',color:'#666',cursor:'pointer',padding:'4px 0',marginBottom:'16px',fontSize:'14px',fontFamily:'inherit'}}>← Back</button>
        <div style={{...card,textAlign:'center',padding:'28px 20px'}}>
          <div style={{display:'flex',justifyContent:'center',marginBottom:'12px',position:'relative'}}>
            <PlayerAvatar player={sp} size={72}/>
            <button onClick={()=>{setAvatarPickerFor(sp.id);setPickerAnimal(sp.avatar);setPickerBg(sp.avatar_bg);}} style={{position:'absolute',bottom:0,right:'calc(50% - 48px)',width:'24px',height:'24px',borderRadius:'50%',background:'#222',border:'1px solid #444',color:'#999',fontSize:'11px',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>✎</button>
          </div>
          <div style={{fontSize:'22px',fontWeight:600}}>{p.name}</div>
          {p.alias&&<div style={{fontSize:'12px',color:'#555',marginTop:'2px'}}>@{(p.alias.includes('@')?p.alias.split('@')[0]:p.alias).toLowerCase()}</div>}
          ...
          <div style={{fontSize:'10px',color:'#555',letterSpacing:'0.1em',marginTop:'-4px'}}>PPR</div>
          ...
          {p.city&&<div style={{fontSize:'11px',color:'#555',marginTop:'12px'}}>{p.city}{p.team?' · '+p.team:''}</div>}
          {isAdmin&&<div style={{display:'flex',gap:'8px',justifyContent:'center',marginTop:'16px'}}><button onClick={()=>openEditPlayer(sp)} style={{padding:'8px 16px',borderRadius:'8px',border:'1px solid #333',background:'#ffffff08',color:'#999',fontSize:'12px',cursor:'pointer',fontFamily:'inherit'}}>Edit Player</button><button onClick={()=>handleDeletePlayer(p.id)} style={{padding:'8px 16px',borderRadius:'8px',border:'1px solid #f8717140',background:'#f8717110',color:'#f87171',fontSize:'12px',cursor:'pointer',fontFamily:'inherit'}}>Delete</button></div>}
        </div>
        {ph.length>1&&<div style={card}><div style={cardH}>Rating History</div><div style={{width:'100%',height:180}}><ResponsiveContainer><LineChart data={ph.map(h=>({...h,label:fmtDate(h.date)}))}><XAxis dataKey="label" tick={{fill:'#555',fontSize:10}} axisLine={{stroke:'#222'}} tickLine={false} interval="preserveStartEnd"/><YAxis tick={{fill:'#555',fontSize:10}} axisLine={false} tickLine={false} domain={['dataMin-20','dataMax+20']}/><Tooltip contentStyle={{background:'#1a1a1a',border:'1px solid #333',borderRadius:'10px',fontSize:'12px'}}/><ReferenceLine y={1500} stroke="#222" strokeDasharray="3 3"/><Line type="monotone" dataKey="rating" stroke="#22d3ee" strokeWidth={2} dot={{r:3,fill:'#22d3ee'}}/></LineChart></ResponsiveContainer></div></div>}
        {Object.keys(opps).length>0&&<div style={card}><div style={cardH}>Head to Head <span style={{color:'#555',fontWeight:400}}>(games)</span></div>{Object.entries(opps).sort((a,b)=>(b[1].wins+b[1].losses)-(a[1].wins+a[1].losses)).map(([oid,rec])=>{const opp=players.find(x=>x.id===oid);if(!opp)return null;const t=rec.wins+rec.losses,pct=t>0?Math.round(rec.wins/t*100):0;return<div key={oid} style={{display:'flex',alignItems:'center',gap:'10px',padding:'10px 0',borderBottom:'1px solid #1e1e1e'}}><PlayerAvatar player={opp} size={32}/><div style={{flex:1}}><div style={{fontSize:'14px',color:'#e5e5e5'}}>{opp.name}</div><div style={{fontSize:'12px',color:'#666'}}>{rec.wins}W – {rec.losses}L</div></div><div style={{width:'60px',height:'4px',borderRadius:'2px',background:'#1e1e1e',overflow:'hidden'}}><div style={{width:`${pct}%`,height:'100%',borderRadius:'2px',background:pct>=50?'#34d399':'#f87171'}}/></div><span style={{fontSize:'12px',color:pct>=50?'#34d399':'#f87171',width:'36px',textAlign:'right',fontVariantNumeric:'tabular-nums'}}>{pct}%</span></div>;})}</div>}
        <div style={card}><div style={cardH}>Match History</div>{pm.map(m=>{const is1=m.player1_id===p.id;const opp=players.find(x=>x.id===(is1?m.player2_id:m.player1_id));const won=is1?m.player1_wins>m.player2_wins:m.player2_wins>m.player1_wins;const sc=is1?`${m.player1_wins}–${m.player2_wins}`:`${m.player2_wins}–${m.player1_wins}`;return<div key={m.id} style={{padding:'10px 0',borderBottom:'1px solid #1e1e1e'}}><div style={{display:'flex',alignItems:'center',gap:'10px'}}><div style={{width:'6px',height:'6px',borderRadius:'50%',background:won?'#34d399':'#f87171',flexShrink:0}}/><div style={{flex:1}}><span style={{fontSize:'14px',color:'#e5e5e5'}}>vs {opp?.name}</span><span style={{fontSize:'13px',color:'#666',marginLeft:'8px'}}>{sc}</span></div>{isAdmin&&<><button onClick={()=>openEditMatch(m)} style={{background:'none',border:'none',color:'#555',cursor:'pointer',fontSize:'11px',padding:'2px 6px',fontFamily:'inherit'}}>✎</button><button onClick={()=>handleDeleteMatch(m.id)} style={{background:'none',border:'none',color:'#333',cursor:'pointer',fontSize:'14px',padding:'2px 6px'}}>×</button></>}</div><div style={{fontSize:'11px',color:'#555',marginTop:'4px',paddingLeft:'16px'}}>{fmtDT(m.created_at)}</div></div>;})}</div>
      </div>
      {/* (3) Avatar picker - animal FIRST then color */}
      {avatarPickerFor&&<div style={overlay} onClick={()=>setAvatarPickerFor(null)}><div style={mdl} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{marginBottom:'14px'}}><label style={lbl}>ANIMAL</label><div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:'8px',maxHeight:'240px',overflowY:'auto'}}>{ANIMAL_KEYS.map(k=><button key={k} onClick={()=>setPickerAnimal(pickerAnimal===k?null:k)} style={{padding:'8px',borderRadius:'10px',background:pickerAnimal===k?'#222':'#0a0a0a',border:pickerAnimal===k?'2px solid #666':'1px solid #1e1e1e',cursor:'pointer',color:'#d4d4d8',display:'flex',flexDirection:'column',alignItems:'center',gap:'4px'}}>{ANIMALS[k](22)}<span style={{fontSize:'9px',color:'#666'}}>{k}</span></button>)}</div></div>
        {pickerAnimal&&<div style={{marginBottom:'20px'}}><label style={lbl}>COLOR</label><div style={{display:'flex',gap:'10px'}}>{BG_COLORS.map(bg=>{const taken=isComboTaken(pickerAnimal,bg.id,avatarPickerFor);return<button key={bg.id} onClick={()=>!taken&&setPickerBg(pickerBg===bg.id?null:bg.id)} disabled={taken} style={{width:'44px',height:'44px',borderRadius:'50%',background:bg.bg,border:pickerBg===bg.id?`3px solid ${bg.border}`:`2px solid ${bg.border}40`,cursor:taken?'not-allowed':'pointer',opacity:taken?.25:1}}/>})}</div></div>}
        <button onClick={handleSaveAvatar} disabled={saving||!pickerAnimal} style={{...btn1,opacity:(saving||!pickerAnimal)?.5:1}}>Save</button>
      </div></div>}
      ...
      {editingMatch&&<div style={overlay} onClick={()=>setEditingMatch(null)}><div style={mdl} onClick={e=>e.stopPropagation()}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'24px'}}><h3 style={{fontSize:'18px',fontWeight:600,color:'#fff',margin:0}}>Edit Score</h3><button onClick={()=>setEditingMatch(null)} style={{background:'none',border:'none',color:'#555',fontSize:'20px',cursor:'pointer'}}>✕</button></div>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'20px',marginBottom:'28px'}}>
          <ScoreStepper label={players.find(x=>x.id===editingMatch.player1_id)?.name?.split(' ')[0]?.toUpperCase()||'P1'} value={emW1} onChange={setEmW1}/>
          <ScoreStepper label={players.find(x=>x.id===editingMatch.player2_id)?.name?.split(' ')[0]?.toUpperCase()||'P2'} value={emW2} onChange={setEmW2}/>
        </div>
        <button onClick={handleEditMatch} disabled={saving||(!emW1&&!emW2)} style={{...btn1,opacity:(saving||(!emW1&&!emW2))?.3:1}}>{saving?'Saving...':'Update Score'}</button>
      </div></div>}
      <style>{`@keyframes slideUp{from{transform:translateY(100%);opacity:0}to{transform:translateY(0);opacity:1}}@keyframes toastIn{from{transform:translate(-50%,-50%) scale(.9);opacity:0}to{transform:translate(-50%,-50%) scale(1);opacity:1}}`}</style>
    </Wrapper>;
  }
  ...
  if(showAllMatches){
    return <Wrapper>
      <div style={{padding:'20px 0 16px'}}><div style={{fontSize:'22px',fontWeight:700,letterSpacing:'-0.03em',color:'#fff'}}>GRAISE <span style={{fontWeight:300,color:'#666'}}>PPR</span></div></div>
      <div style={{paddingTop:'8px',paddingBottom:'40px'}}>
        <button onClick={()=>setShowAllMatches(false)} style={{background:'none',border:'none',color:'#666',cursor:'pointer',padding:'4px 0',marginBottom:'16px',fontSize:'14px',fontFamily:'inherit'}}>← Back</button>
        <div style={card}><div style={cardH}>All Matches</div>{[...sortedMatches].reverse().map(m=>{const p1=players.find(p=>p.id===m.player1_id),p2=players.find(p=>p.id===m.player2_id);return<div key={m.id} style={{padding:'12px 0',borderBottom:'1px solid #1e1e1e'}}><div style={{display:'flex',alignItems:'center',gap:'10px'}}><PlayerAvatar player={p1} size={28}/><div style={{flex:1,display:'flex',alignItems:'center',gap:'8px',flexWrap:'wrap'}}><span style={{fontSize:'14px',color:m.player1_wins>m.player2_wins?'#34d399':'#e5e5e5',fontWeight:m.player1_wins>m.player2_wins?500:400}}>{p1?.name}</span><span style={{fontSize:'14px',color:'#666',fontVariantNumeric:'tabular-nums'}}>{m.player1_wins}–{m.player2_wins}</span><span style={{fontSize:'14px',color:m.player2_wins>m.player1_wins?'#34d399':'#e5e5e5',fontWeight:m.player2_wins>m.player1_wins?500:400}}>{p2?.name}</span></div><PlayerAvatar player={p2} size={28}/>{isAdmin&&<><button onClick={()=>openEditMatch(m)} style={{background:'none',border:'none',color:'#555',cursor:'pointer',fontSize:'11px',padding:'2px 6px',fontFamily:'inherit'}}>✎</button><button onClick={()=>handleDeleteMatch(m.id)} style={{background:'none',border:'none',color:'#444',cursor:'pointer',fontSize:'14px',padding:'2px 6px'}}>×</button></>}</div><div style={{fontSize:'11px',color:'#555',marginTop:'6px'}}>{fmtDT(m.created_at)}</div></div>;})}</div>
      </div>
      {editingMatch&&<div style={overlay} onClick={()=>setEditingMatch(null)}><div style={mdl} onClick={e=>e.stopPropagation()}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'24px'}}><h3 style={{fontSize:'18px',fontWeight:600,color:'#fff',margin:0}}>Edit Score</h3><button onClick={()=>setEditingMatch(null)} style={{background:'none',border:'none',color:'#555',fontSize:'20px',cursor:'pointer'}}>✕</button></div>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'20px',marginBottom:'28px'}}>
  ...
  // ═══ HOME ══════════════════════════════════════════════════════
  return <Wrapper>
    {/* (7) More readable header */}
    <div style={{padding:'20px 0 16px',display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:50,background:'linear-gradient(to bottom, #050505 60%, transparent)'}}>
      <div>
        <div style={{fontSize:'22px',fontWeight:700,letterSpacing:'-0.03em',color:'#fff'}}>GRAISE <span style={{fontWeight:300,color:'#666'}}>PPR</span></div>
        <div style={{fontSize:'10px',color:'#555',letterSpacing:'0.1em',marginTop:'2px',fontWeight:500}}>PING PONG RATINGS</div>
      </div>
      <div style={{display:'flex',gap:'8px',alignItems:'center'}}>
        ...
        {!isAdmin?<button onClick={()=>setShowAdmin(true)} style={{width:'32px',height:'32px',borderRadius:'50%',border:'1px solid #333',background:'transparent',color:'#555',cursor:'pointer',fontSize:'14px',display:'flex',alignItems:'center',justifyContent:'center'}}>⚙</button>:<button onClick={()=>setIsAdmin(false)} style={{padding:'6px 12px',borderRadius:'14px',border:'1px solid #f8717130',background:'#f8717110',color:'#f87171',cursor:'pointer',fontSize:'11px',fontFamily:'inherit'}}>Admin ✕</button>}
      </div>
    </div>

    <div style={{paddingTop:'8px',paddingBottom:'40px'}}>

      {/* Rankings - (1) shows ALL players */}
      <div style={card}><div style={cardH}>Rankings</div>
        {leaderboard.map((p,idx)=>{const momentum=getMomentum(p.id,sortedMatches);const streak=getStreak(p.id,sortedMatches);const ti=p.totalMatches>0?getTitleForRank(idx+1,leaderboard.length):{title:'New Player',color:'#444'};return<div key={p.id} onClick={()=>setSelectedPlayer(p)} style={{display:'flex',alignItems:'center',gap:'12px',padding:'12px 0',borderBottom:idx<leaderboard.length-1?'1px solid #1e1e1e':'none',cursor:'pointer'}}>
          <span style={{width:'22px',fontSize:'13px',fontWeight:600,color:p.totalMatches===0?'#333':idx===0?'#fbbf24':idx===1?'#94a3b8':idx===2?'#d97706':'#555',textAlign:'center',flexShrink:0,fontVariantNumeric:'tabular-nums'}}>{p.totalMatches>0?idx+1:'—'}</span>
          <PlayerAvatar player={p} size={40}/>
          <div style={{flex:1,minWidth:0}}>
            <div style={{display:'flex',alignItems:'center',gap:'8px'}}><span style={{fontSize:'15px',fontWeight:500,color:'#e5e5e5'}}>{p.name}</span>{streak>=3&&<span style={{fontSize:'12px',color:'#f97316'}}>🔥{streak}</span>}</div>
            <div style={{display:'flex',alignItems:'center',gap:'10px',marginTop:'3px'}}><span style={{fontSize:'11px',color:ti.color}}>{ti.title}</span>{p.totalMatches>0&&<><span style={{fontSize:'12px',color:'#555'}}>{p.wins}W–{p.losses}L</span>{momentum.length>0&&<MomentumDots results={momentum}/>}</>}</div>
          </div>
          <div style={{textAlign:'right',flexShrink:0}}><div style={{fontSize:'18px',fontWeight:300,color:p.totalMatches>0?'#fff':'#444',fontVariantNumeric:'tabular-nums'}}>{p.rating}</div><div style={{fontSize:'10px',color:'#555',letterSpacing:'0.05em'}}>PPR</div></div>
        </div>;})}
        {leaderboard.length===0&&<div style={{color:'#555',fontSize:'14px',textAlign:'center',padding:'30px 0'}}>No players yet. Add some!</div>}
      </div>

      <button onClick={()=>setShowAddPlayer(true)} style={{width:'100%',padding:'14px',borderRadius:'12px',border:'1px dashed #333',background:'transparent',color:'#555',fontSize:'14px',cursor:'pointer',marginBottom:'12px',fontFamily:'inherit'}}>+ Add Player</button>

      {/* Rating History */}
      {chartData.length>1&&<div style={card}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'14px'}}>
          <div style={{...cardH,marginBottom:0}}>Rating History</div>
        ...
        <div style={{width:'100%',height:220}}><ResponsiveContainer><LineChart data={chartData}>
          <XAxis dataKey="label" tick={{fill:'#555',fontSize:10}} axisLine={{stroke:'#222'}} tickLine={false} interval="preserveStartEnd"/>
          <YAxis tick={{fill:'#555',fontSize:10}} axisLine={false} tickLine={false} domain={['dataMin-20','dataMax+20']}/>
          <Tooltip contentStyle={{background:'#1a1a1a',border:'1px solid #333',borderRadius:'10px',fontSize:'12px'}}/>
          <ReferenceLine y={1500} stroke="#222" strokeDasharray="3 3"/>
          {playersWithMatches.slice(0,8).map((p,i)=><Line key={p.id} type="monotone" dataKey={p.name} stroke={PLAYER_COLORS[i%PLAYER_COLORS.length]} strokeWidth={2} dot={false} connectNulls={false}/>)}
        </LineChart></ResponsiveContainer></div>
        <div style={{display:'flex',flexWrap:'wrap',gap:'8px 16px',marginTop:'12px'}}>{playersWithMatches.slice(0,8).map((p,i)=><div key={p.id} style={{display:'flex',alignItems:'center',gap:'6px'}}><div style={{width:'8px',height:'8px',borderRadius:'50%',background:PLAYER_COLORS[i%PLAYER_COLORS.length]}}/><span style={{fontSize:'11px',color:'#888'}}>{p.name}</span></div>)}</div>
      </div>}

      {/* Recent Matches - (2) with date+time */}
      <div style={card}><div style={cardH}>Recent Matches</div>
        {[...sortedMatches].reverse().slice(0,5).map(m=>{const p1=players.find(p=>p.id===m.player1_id),p2=players.find(p=>p.id===m.player2_id);return<div key={m.id} style={{padding:'10px 0',borderBottom:'1px solid #1e1e1e'}}>
          <div style={{display:'flex',alignItems:'center',gap:'10px'}}><PlayerAvatar player={p1} size={28}/><div style={{flex:1,display:'flex',alignItems:'center',gap:'8px',flexWrap:'wrap'}}><span style={{fontSize:'14px',color:m.player1_wins>m.player2_wins?'#34d399':'#e5e5e5',fontWeight:m.player1_wins>m.player2_wins?500:400}}>{p1?.name}</span><span style={{fontSize:'14px',color:'#666',fontVariantNumeric:'tabular-nums'}}>{m.player1_wins}–{m.player2_wins}</span><span style={{fontSize:'14px',color:m.player2_wins>m.player1_wins?'#34d399':'#e5e5e5',fontWeight:m.player2_wins>m.player1_wins?500:400}}>{p2?.name}</span></div><PlayerAvatar player={p2} size={28}/></div>
          <div style={{fontSize:'11px',color:'#555',marginTop:'6px',paddingLeft:'38px'}}>{fmtDT(m.created_at)}</div>
        </div>;})}
        {sortedMatches.length>5&&<button onClick={()=>setShowAllMatches(true)} style={{width:'100%',padding:'12px',marginTop:'8px',borderRadius:'10px',border:'1px solid #222',background:'transparent',color:'#888',fontSize:'13px',cursor:'pointer',fontFamily:'inherit'}}>See all {matches.length} matches →</button>}
      </div>

      {/* Highlights - (5) removed Wall, added Connections Built */}
      {matches.length>0&&<div style={card}><div style={cardH}>Highlights</div>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
          {funStats.hotStreak&&<div style={{background:'#0a0a0a',borderRadius:'10px',padding:'14px',border:'1px solid #1a1a1a'}}><div style={{fontSize:'10px',color:'#666',letterSpacing:'0.05em',marginBottom:'6px'}}>🔥 HOT STREAK</div><div style={{fontSize:'14px',color:'#f97316',fontWeight:500}}>{funStats.hotStreak.player.name}</div><div style={{fontSize:'12px',color:'#777',marginTop:'2px'}}>{funStats.hotStreak.streak} straight wins</div></div>}
          {funStats.topGainerWeek&&funStats.topGainerWeek.gain>0&&<div style={{background:'#0a0a0a',borderRadius:'10px',padding:'14px',border:'1px solid #1a1a1a'}}><div style={{fontSize:'10px',color:'#666',letterSpacing:'0.05em',marginBottom:'6px'}}>📈 RISING STAR</div><div style={{fontSize:'14px',color:'#34d399',fontWeight:500}}>{funStats.topGainerWeek.player.name}</div><div style={{fontSize:'12px',color:'#777',marginTop:'2px'}}>+{funStats.topGainerWeek.gain} PPR this week</div></div>}
          {funStats.topGainerMonth&&funStats.topGainerMonth.gain>0&&<div style={{background:'#0a0a0a',borderRadius:'10px',padding:'14px',border:'1px solid #1a1a1a'}}><div style={{fontSize:'10px',color:'#666',letterSpacing:'0.05em',marginBottom:'6px'}}>🚀 MONTHLY MVP</div><div style={{fontSize:'14px',color:'#34d399',fontWeight:500}}>{funStats.topGainerMonth.player.name}</div><div style={{fontSize:'12px',color:'#777',marginTop:'2px'}}>+{funStats.topGainerMonth.gain} PPR this month</div></div>}
          {funStats.biggestFallWeek&&funStats.biggestFallWeek.gain<0&&<div style={{background:'#0a0a0a',borderRadius:'10px',padding:'14px',border:'1px solid #1a1a1a'}}><div style={{fontSize:'10px',color:'#666',letterSpacing:'0.05em',marginBottom:'6px'}}>🎢 REDEMPTION ARC</div><div style={{fontSize:'14px',color:'#f472b6',fontWeight:500}}>{funStats.biggestFallWeek.player.name}</div><div style={{fontSize:'12px',color:'#777',marginTop:'2px'}}>{funStats.biggestFallWeek.gain} PPR this week</div></div>}
          {funStats.biggestUpset&&<div style={{background:'#0a0a0a',borderRadius:'10px',padding:'14px',border:'1px solid #1a1a1a'}}><div style={{fontSize:'10px',color:'#666',letterSpacing:'0.05em',marginBottom:'6px'}}>😱 BIGGEST UPSET</div><div style={{fontSize:'14px',color:'#a78bfa',fontWeight:500}}>{funStats.biggestUpset.winner.name}</div><div style={{fontSize:'12px',color:'#777',marginTop:'2px'}}>beat {funStats.biggestUpset.loser.name} ({funStats.biggestUpset.diff} PPR gap)</div></div>}
          {funStats.nemesis&&funStats.nemesis.count>=2&&<div style={{background:'#0a0a0a',borderRadius:'10px',padding:'14px',border:'1px solid #1a1a1a'}}><div style={{fontSize:'10px',color:'#666',letterSpacing:'0.05em',marginBottom:'6px'}}>🧲 ARCH RIVALS</div><div style={{fontSize:'14px',color:'#fbbf24',fontWeight:500}}>{funStats.nemesis.p1?.name} vs {funStats.nemesis.p2?.name}</div><div style={{fontSize:'12px',color:'#777',marginTop:'2px'}}>{funStats.nemesis.count} matches played</div></div>}
          {funStats.shutoutKing&&funStats.shutoutKing.count>=2&&<div style={{background:'#0a0a0a',borderRadius:'10px',padding:'14px',border:'1px solid #1a1a1a'}}><div style={{fontSize:'10px',color:'#666',letterSpacing:'0.05em',marginBottom:'6px'}}>💀 SHUTOUT KING</div><div style={{fontSize:'14px',color:'#f87171',fontWeight:500}}>{funStats.shutoutKing.player.name}</div><div style={{fontSize:'12px',color:'#777',marginTop:'2px'}}>{funStats.shutoutKing.count} perfect wins</div></div>}
          {funStats.clutchPlayer&&<div style={{background:'#0a0a0a',borderRadius:'10px',padding:'14px',border:'1px solid #1a1a1a'}}><div style={{fontSize:'10px',color:'#666',letterSpacing:'0.05em',marginBottom:'6px'}}>🎯 GIANT KILLER</div><div style={{fontSize:'14px',color:'#60a5fa',fontWeight:500}}>{funStats.clutchPlayer.player.name}</div><div style={{fontSize:'12px',color:'#777',marginTop:'2px'}}>{funStats.clutchPlayer.count} underdog wins</div></div>}
          <div style={{background:'#0a0a0a',borderRadius:'10px',padding:'14px',border:'1px solid #1a1a1a'}}><div style={{fontSize:'10px',color:'#666',letterSpacing:'0.05em',marginBottom:'6px'}}>🤝 CONNECTIONS BUILT</div><div style={{fontSize:'14px',color:'#22d3ee',fontWeight:500}}>{funStats.uniquePairs} pairings</div><div style={{fontSize:'12px',color:'#777',marginTop:'2px'}}>{funStats.totalMatches} matches played</div></div>
          <div style={{background:'#0a0a0a',borderRadius:'10px',padding:'14px',border:'1px solid #1a1a1a'}}><div style={{fontSize:'10px',color:'#666',letterSpacing:'0.05em',marginBottom:'6px'}}>🏓 THE TALLY</div><div style={{fontSize:'14px',color:'#e5e5e5',fontWeight:500}}>{funStats.totalGames} games</div><div style={{fontSize:'12px',color:'#777',marginTop:'2px'}}>{funStats.totalMatches} matches played</div></div>
        </div>
      </div>}

      {/* Head to Head - (6) uses games not matches */}
      {playersWithMatches.length>=2&&<div style={card}><div style={cardH}>Head to Head <span style={{color:'#555',fontWeight:400}}>(games)</span></div>
        <div style={{overflowX:'auto',WebkitOverflowScrolling:'touch'}}>
          <table style={{width:'100%',borderCollapse:'collapse',fontSize:'12px'}}>
            <thead><tr><th style={{padding:'6px',textAlign:'left',color:'#666',fontWeight:400}}></th>{playersWithMatches.map(p=><th key={p.id} style={{padding:'6px',color:'#999',fontWeight:400,fontSize:'11px',whiteSpace:'nowrap'}}>{p.name.split(' ')[0].slice(0,6)}</th>)}</tr></thead>
            <tbody>{playersWithMatches.map(p1=><tr key={p1.id}><td style={{padding:'6px',color:'#999',fontSize:'11px',whiteSpace:'nowrap'}}>{p1.name.split(' ')[0].slice(0,6)}</td>
              {playersWithMatches.map(p2=>{if(p1.id===p2.id)return<td key={p2.id} style={{padding:'6px',textAlign:'center',color:'#222'}}>—</td>;
                let gw=0,gl=0;
                sortedMatches.forEach(m=>{
                  if(m.player1_id===p1.id&&m.player2_id===p2.id){gw+=m.player1_wins;gl+=m.player2_wins;}
                  else if(m.player2_id===p1.id&&m.player1_id===p2.id){gw+=m.player2_wins;gl+=m.player1_wins;}
                });
                return<td key={p2.id} style={{padding:'6px',textAlign:'center'}}>{gw+gl>0?<span style={{color:gw>gl?'#34d399':gw<gl?'#f87171':'#888'}}>{gw}–{gl}</span>:<span style={{color:'#222'}}>·</span>}</td>;})}
            </tr>)}</tbody>
          </table>
        </div>
      </div>}
    ...
    {showAddMatch&&<div style={overlay} onClick={()=>setShowAddMatch(false)}><div style={mdl} onClick={e=>e.stopPropagation()}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'24px'}}><h3 style={{fontSize:'18px',fontWeight:600,color:'#fff',margin:0}}>Record Match</h3><button onClick={()=>setShowAddMatch(false)} style={{background:'none',border:'none',color:'#555',fontSize:'20px',cursor:'pointer'}}>✕</button></div>
    ...
    {showAddPlayer&&<div style={overlay} onClick={()=>setShowAddPlayer(false)}><div style={mdl} onClick={e=>e.stopPropagation()}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'24px'}}><h3 style={{fontSize:'18px',fontWeight:600,color:'#fff',margin:0}}>New Player</h3><button onClick={()=>setShowAddPlayer(false)} style={{background:'none',border:'none',color:'#555',fontSize:'20px',cursor:'pointer'}}>✕</button></div>
      ...
      <div style={{marginBottom:'14px'}}><label style={lbl}>ALIAS (FOR SLACK)</label><input value={npAlias} onChange={e=>setNpAlias(e.target.value)} placeholder="@handle or nickname" style={inp}/></div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px',marginBottom:'14px'}}>
      ...
      {/* (3) Avatar: animal first, then color */}
      <div style={{marginBottom:'14px'}}><label style={lbl}>AVATAR (OPTIONAL)</label>
        <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:'8px',maxHeight:'200px',overflowY:'auto',marginBottom:'12px'}}>{ANIMAL_KEYS.map(k=>{const s2=npAvatar===k;return<button key={k} onClick={()=>setNpAvatar(s2?null:k)} style={{padding:'8px',borderRadius:'10px',background:s2?'#222':'#0a0a0a',border:s2?'2px solid #666':'1px solid #1e1e1e',cursor:'pointer',color:'#d4d4d8',display:'flex',flexDirection:'column',alignItems:'center',gap:'4px'}}>{ANIMALS[k](22)}<span style={{fontSize:'9px',color:'#666'}}>{k}</span></button>;})}</div>
        {npAvatar?<div><label style={lbl}>COLOR</label><div style={{display:'flex',gap:'10px'}}>{BG_COLORS.map(bg=>{const taken=isComboTaken(npAvatar,bg.id,null);return<button key={bg.id} onClick={()=>!taken&&setNpAvatarBg(npAvatarBg===bg.id?null:bg.id)} disabled={taken} style={{width:'44px',height:'44px',borderRadius:'50%',background:bg.bg,border:npAvatarBg===bg.id?`3px solid ${bg.border}`:`2px solid ${bg.border}40`,cursor:taken?'not-allowed':'pointer',opacity:taken?.25:1}}/>})}</div></div>:<div style={{fontSize:'12px',color:'#555'}}>Pick an animal first, or skip for initials</div>}
      </div>
